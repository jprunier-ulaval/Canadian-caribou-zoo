...
## Summary:
The model to predict ecotypes from genotypic data only was developped and optimized using the script _ML-ecotype3.py_. The development was based on a typical machine-learning approach where the data set was first split into test and learning sets. Then a cross-validation procedure was used to train the model and test various hyper-parameters. The final model performance was evaluated on the independent test set where the accurracy reached 100%. This final model was saved and used in the second analysis (scripted in _predict-zoo-samples_) to predict the ecotype of zoo samples based on genotypes only.
